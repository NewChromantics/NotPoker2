import {CreatePromise,PromiseQueue,Yield} from './PromiseQueue.js'
...
{
	if ( typeof GameType != typeof '' )
		throw `Game Type(${GameType}) needs to be a string`;
	
	if ( GameType.length < 4 )
		throw `Game Type(${GameType}) needs to be at least 4 chars`;
...
export default class TGame
{
	...
	{
		//	cut player out
		this.OnDebug(`Players before delete; ${this.Players} ... deleting ${PlayerHash}`);
	...
	GetCurrentPlayer()
	{
		...
		{
			this.OnDebug(`GetCurrentPlayer NextPlayer = ${this.NextPlayer}`);
		...
		for ( let i=0;	i<this.Players.length;	i++ )
		{
			...
			this.OnDebug(`GetCurrentPlayer: ${Player} not played, is next`);
			return Player;
		}
		
		//	everyone has played, take the oldest one and return it
		this.OnDebug(`GetCurrentPlayer: using oldest players; ${this.LastPlayers}`);
	...
	EndPlayerTurn(ExpectedPlayer=null)
	{
		...
		this.OnDebug(`Ending player's turn ${this.NextPlayer}`);
