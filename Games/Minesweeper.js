...
function GetNeighbourCount(x,y,Map)
{
	...
	//this.OnDebug(`Counted ${CheckedMines} == ${MineCount}`);
...
function InitGameMap(Size,SafePosition,MineCount)
{
	...
	//this.OnDebug(`Map with GetNeighbourCount: ${JSON.stringify(Map,null,'\t')}`);
	return Map;
}
	...
	async RunGame(SendMoveAndWait,OnStateChanged,OnAction)
	{
		while ( !this.GetWinner() )
		{
			...
			try
			{
				...
				this.OnDebug(`${Player} WaitPlayerPickCoord PickedBomb = ${PickedBomb}`);
				if ( !PickedBomb )
				{
					OnStateChanged();
